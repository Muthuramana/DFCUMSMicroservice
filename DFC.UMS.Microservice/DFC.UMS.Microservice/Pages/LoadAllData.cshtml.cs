...
namespace DFC.UMS.Microservice.Pages
{
    public class LoadAllDataModel : PageModel
    {
        ...
        public IEnumerable<StepDetail> StepDetails { get; set; } = new List<StepDetail>{ new StepDetail{QuestionId  = 1, QuestionDescription = "Skills you have are comfortable with" , FrameworkItemType = nameof(Skill)}, new StepDetail { QuestionId = 3, QuestionDescription = "Tasks you would enjoy in the work environment", FrameworkItemType = nameof(TaskItem) } , new StepDetail { QuestionId = 2, QuestionDescription = "Abilities you are willingly to improve", FrameworkItemType = nameof(Ability) } };
